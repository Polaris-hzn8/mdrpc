...
#### rpc服务提供者UserSeriviceRpc

1. protobuff不支持rpc方法，只支持对rpc方法的描述，
2. 通过这些描述就可以去做rpc请求所携带参数的序列化与反序列化，
...
#### rpc服务消费者UserSeriviceRpc_Stub

1. 没有默认构造函数，构造对象时必须传入RpcChannel对象指针，
2. 并且对应的具体的函数并非虚函数，
3. 不论用Stub类调用任何方法，最终都是转为调用RpcChannel对象的CallMethod方法
...
RpcChannel抽象类内部全为纯虚函数，

必须实现一个类继承该抽象类，并重写对应的纯虚函数方法：

```cpp
// 自实现类继承自抽象类
class MyRpcChannel : public RpcChannel {
    // 实现纯虚函数
    void CallMethod(const MethodDescriptor* method,	// 调用的方案
                    RpcController* controller,		// 
                    const Message* request,			// 请求参数
                    Message* response,				// 响应结果
                    Closure* done) {
        // TODO
        // 1.RPC方法的序列化
        // 2.发起远程的RPC调用请求
...
1. message类型：用于rpc方法参数与返回值的序列化与反序列化
2. service类型：生成两个类UserSeriviceRpc、UserSeriviceRpc_Stub
   - UserSeriviceRpc：用于rpc服务提供者
   - UserSeriviceRpc_Stub：用于rpc服务调用者
